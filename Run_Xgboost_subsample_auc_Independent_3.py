...
data_mat = scipy.io.loadmat('./JS_QC_data3_RC_ag_feature_subset.mat')
datapath = '../DataL12/'

A = data_mat['Xrm_sub']
Y = data_mat['Yrm']
CV_ID = data_mat['CV_IDrm_rc']
tr_id = data_mat['tr_idrm_new']
print A.shape
...
final_perf[count,:]= a
count += 1
#np.savetxt('JS_QC_RM3_independent_test_subfeature.txt', Ycp)

A = data_mat['Xrd_sub']
Y = data_mat['Yrd']
CV_ID = data_mat['CV_IDrd_rc']
tr_id = data_mat['tr_idrd_new']
X = np.array(stats.zscore(A, axis=0, ddof=1))
print A.shape
a, Ycp = Unbalanced_classify(X, Y, CV_ID, tr_id, param, 30)
final_perf[count,:]= a
